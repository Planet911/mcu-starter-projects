...
HAL_GPIO_PIN(VCP_STATUS,       C, 6) // LED
...
static void sys_init(void)
{
  ...
  while (0 == (RCC->CRRCR & RCC_CRRCR_HSI48RDY));

  //RCC->CR = RCC_CR_HSEON | RCC_CR_HSEBYP; //External oscillator module
  RCC->CR = RCC_CR_HSEON; //External crystal
  ...
  while ( ( RCC->CFGR & RCC_CFGR_SWS ) >> RCC_CFGR_SWS_Pos != 3/*PLL*/ );
  // System clock is now 112 MHz (with 8MHz crystal).
}
